...
// 父组件接收的标签列表
const { valueModel, updateOriginTag } = defineProps({
  valueModel: { default: '' },
  updateOriginTag: { default: '' },
})
// 同步 选中的标签列表 至 父组件
...
onMounted(() => {
  // 加载所有标签
  loadTags()
  // 如果有传值说明是修改文章 需要展示原有标签
  tagsRef.value = Boolean(updateOriginTag.length) ?
    JSON.parse(updateOriginTag)
    : []
